...
18.请根据你的理解，从内核层和框架层两方面出发，描述一下Android的安全机制，并以申请android.permission.INTERNET权限为例子，描述一下这个机制是如何工作的。
...
:1E
  ...
  goto                :12
.end method
输入str=“Helloworld", i=124，请问返回值是多少？

25.运行前R0 = 0xBABBBD,那么编译为ARM指令运行下段汇编后，R0寄存器的值是______
stmfd   sp!,{r0,r1,ip} 
ldr     ip,LtmpPAddr 
sub     ip,#4
mov     r0,ip
